...
import type { AppData, Phrase, Rating, Session, LearningState, ExposureSession, ExposurePack, ExposurePhrase, SavedPhrase } from './types';
...
import { allPacks, becPacks, findPack } from './data/bec';
...
export default function App() {
  ...
  function startPack(pack: ExposurePack = suggestedPack) {
    setData(d => openPack(d, pack.id, today));
    setPage('exposure');
  }
  function changeExposure(patch: Partial<ExposureSession>) {
    setData(d => { if (!d.exposureSession) return d; const session = { ...d.exposureSession, ...patch }; return { ...d, exposureSession: session, exposureDrafts: { ...d.exposureDrafts, [session.packId]: session } }; });
  }
  function recordExposure() {
    setData(d => {
      const s = d.exposureSession;
      if (!s) return d;
      const record = { id: s.id, packId: s.packId, date: s.date, stages: s.visited, at: new Date().toISOString() };
      return { ...d, exposureHistory: [...(d.exposureHistory ?? []).filter(h => h.id !== s.id), record] };
    });
  }
  function finishPack() { recordExposure(); setPage('exposure-done'); }
  function saveExposurePhrase(phrase: ExposurePhrase, state?: LearningState) {
    setData(d => {
      const existing = d.phrases.find(p => p.phrase === phrase.phrase);
      const saved = { ...phrase, id: crypto.randomUUID(), note: '', savedAt: new Date().toISOString(), topic: activePack?.topic, sourceTitle: activePack?.reading?.title ?? activePack?.title, sourceId: activePack?.id, ...existing };
      const updated = state ? markPhrase(saved, state) : saved;
      return { ...d, phrases: existing ? d.phrases.map(p => p.id === existing.id ? updated : p) : [...d.phrases, updated] };
    });
    setNotice(state ? 'Phrase saved · ' + state : 'Phrase saved. Choose a learning state whenever you are ready.');
  }
  function savePersonalPhrase(phrase: SavedPhrase) {
    setData(d => d.phrases.some(p => p.phrase.toLowerCase() === phrase.phrase.toLowerCase() && p.sourceId === phrase.sourceId) ? d : { ...d, phrases: [...d.phrases, phrase] });
  }
  function startDaily() {
  ...
  return <div className={`shell ${(page === 'drill' || page === 'exposure') ? 'in-drill' : ''}`}><a className="skip-link" href="#main">Skip to content</a><header className="header"><button className="brand" onClick={() => setPage('home')} aria-label="Business English Drill home"><img src={`${import.meta.env.BASE_URL}favicon.svg`} alt=""/><span>Business English<span className="brand-sub">DRILL</span></span></button><span className="header-note">Small practice. Real conversations.</span></header>
    <nav className="navigation" aria-label="Main navigation">{navigation.map(item => <button key={item.page} aria-current={page === item.page || (item.page === 'home' && ((page === 'drill' || page === 'done') && mode === 'daily' || page === 'exposure' || page === 'exposure-done')) ? 'page' : undefined} onClick={() => setPage(item.page)}><Icon name={item.icon}/><span>{item.label}</span>{item.page === 'review' && (due.length + phraseDue.length) > 0 && <span className="nav-count">{due.length + phraseDue.length}</span>}</button>)}</nav>
    ...
    <main id="main" ref={main} tabIndex={-1}>
      ...
      {page === 'exposure' && data.exposureSession && activePack && <Exposure key={activePack.id} pack={activePack} session={data.exposureSession} phrases={data.phrases} rescue={data.rescue} onChange={changeExposure} onSave={saveExposurePhrase} onFinish={finishPack} onLegacy={() => { recordExposure(); startDaily(); }}/>}
      {page === 'exposure-done' && <div className="done"><div className="done-mark"><Icon name="check" size={38}/></div><p className="eyebrow">ENOUGH FOR TODAY</p><h1>A little more familiar.</h1><p>{storageError ? 'Your changes are not saved. Keep this page open and export in Settings.' : 'Your learning visit is saved on this device.'}<br/>You do not need to produce an answer to make progress.</p><p className="quiet">Explored: {data.exposureSession?.visited.join(' · ')}. Visiting a stage does not mean mastering it.</p><div className="support-actions"><button className="primary" onClick={() => setPage('home')}>Back to today</button><button className="secondary" onClick={() => startPack(activePack ?? suggestedPack)}>Continue exploring</button></div></div>}
